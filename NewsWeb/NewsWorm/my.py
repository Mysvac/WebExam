...
class NewsInfo:
    ...
    def save_mysql(self, time, title,  picture_url):
        data = list(zip(time, title, picture_url))
        sql = "INSERT INTO news_info (time, title,  picture_url)VALUES (%s,%s,%s);";
        self.conn = pymysql.connect(**mysql_local)
        self.cursor = self.conn.cursor()
    ...
    def main(self):
        ...
        for num in range(1, 11):
            ...
            self.save_mysql(time, title,  picture_url)
